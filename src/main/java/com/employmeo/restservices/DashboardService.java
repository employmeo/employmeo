package com.employmeo.restservices;
...
public class DashboardService {
			    ...
			    )
	  {
		  ...
		  String sql = "SELECT r.respondantStatus, r.respondantProfile, COUNT(r) from Respondant r GROUP BY r.respondantStatus, r.respondantProfile";
		  Query query = em.createQuery(sql);

		  List<Object[]> results = query.getResultList();
		  List<String> labels = Arrays.asList("unscored",PositionProfile.PROFILE_A,PositionProfile.PROFILE_B,PositionProfile.PROFILE_C,PositionProfile.PROFILE_D);
		  int totalInvited = 0;
		  int totalStarted = 0;
		  ...
		  for (int i=0;i<labels.size();i++) {
			  ...
			  appDatasets.accumulate("backgroundColor", PositionProfile.getProfileDefaults(labels.get(i)).get("profile_color"));
			  appDatasets.accumulate("hoverBackgroundColor", PositionProfile.getProfileDefaults(labels.get(i)).get("profile_highlight"));
		  }
		  ...
		  for (int i=0;i<labels.size();i++) {
			  ...
			  hireDatasets.accumulate("backgroundColor", PositionProfile.getProfileDefaults(labels.get(i)).get("profile_color"));
			  hireDatasets.accumulate("hoverBackgroundColor", PositionProfile.getProfileDefaults(labels.get(i)).get("profile_highlight"));
		  }
